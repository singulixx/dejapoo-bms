...
export async function syncShopeeOrders(opts?: { hours?: number }) {
  const lookbackHoursRaw = Number(opts?.hours ?? 24);
  const lookbackHours = Number.isFinite(lookbackHoursRaw) && lookbackHoursRaw > 0 && lookbackHoursRaw <= 168 ? lookbackHoursRaw : 24;
  ...
  if (!accounts.length) {
    return { ok: true, message: "No active Shopee accounts", results: [] as any[] };
  }
      ...
      for (const orderSn of orderSns) {
        const detail = await shopeeAuthedGet<any>({
          ...
          query: {
            ...
            response_optional_fields:
              "item_list,recipient_address,shipping_carrier,total_amount,actual_shipping_fee,package_list",
          },
        });
        ...
        const items = itemList
          .map((it) => {
            ...
            return { externalSkuId, qty, price: price != null ? Number(price) : null };
          })
  ...
  return { ok: true, range: { hours: lookbackHours }, results };
}

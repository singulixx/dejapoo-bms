...
export async function POST(req: Request) {
  ...
  // ✅ Normalisasi: RequestContext butuh undefined, bukan null
  const ctx = {
    ...auth.ctx,
    req: {
      ...auth.ctx.req,
      path: auth.ctx.req.path ?? undefined,
      ip: auth.ctx.req.ip ?? undefined,
      userAgent: auth.ctx.req.userAgent ?? undefined,
    },
  };
  ...
  return await requestContext.run(ctx, async () => {
    ...
    // Explicit audit log (route-level) to guarantee "who did what"
    if (created?.id) {

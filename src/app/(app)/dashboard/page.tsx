...
type Summary = {
  ...
  trends: {
    ...
    daily: {
      date: string;
      total: { orders: number; amount: number };
      shopee: { orders: number; amount: number };
      tiktok: { orders: number; amount: number };
      offline: { orders: number; amount: number };
    }[];
  };
  outletPerformance30d: { outletName: string; inQty: number; outQty: number }[];
};
...
export default function DashboardPage() {
  ...
  const trendCategories = useMemo(() => (data?.trends.daily ?? []).map((x) => x.date.slice(5)), [data]);
  const trendSeries = useMemo(() => {
    const rows = data?.trends.daily ?? [];
    return [
      { name: "Total", data: rows.map((x) => x.total.amount) },
      { name: "Shopee", data: rows.map((x) => x.shopee.amount) },
      { name: "TikTok", data: rows.map((x) => x.tiktok.amount) },
      { name: "Offline", data: rows.map((x) => x.offline.amount) },
    ];
  }, [data]);
  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-stroke dark:border-white/10 bg-card dark:bg-card/5 p-4">
          <div className="mb-1 text-sm text-dark-5 dark:text-white/70">Tren Penjualan (30 hari)</div>
          <div className="mb-3 text-xs text-dark-6 dark:text-white/40">Total & per channel (Shopee/TikTok/Offline)</div>
          <Chart
            type="line"
            height={280}
      ...
      <div className="rounded-2xl border border-stroke dark:border-white/10 bg-card dark:bg-card/5 p-4">
        <div className="mb-3 text-sm text-dark-5 dark:text-white/70">Kinerja Outlet (30 hari)</div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-dark dark:text-white/90">
            ...
            <tbody>
              {data.outletPerformance30d.map((r, idx) => (
                <tr key={idx} className="border-t border-stroke dark:border-white/10">
                  <td className="py-2">{r.outletName}</td>
                  <td className="py-2 text-right">{r.inQty}</td>
                  <td className="py-2 text-right">{r.outQty}</td>
                </tr>
              ))}
            </tbody>
          </table>

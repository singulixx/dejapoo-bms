...
type MarketplaceAccount = { id: string; channel: Channel; name?: string | null; isActive: boolean };
...
export default function MarketplacePage() {
  ...
  async function load() {
    ...
    setAccounts((j?.items ?? []) as MarketplaceAccount[]);
    setLoading(false);
  }
    ...
    <div className="space-y-6 p-4 md:p-6">
      <div>
        ...
        <div className="text-sm text-dark-5 dark:text-white/60">
          Connect akun + mapping SKU (UI stub)
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-stroke dark:border-white/10 bg-card dark:bg-card/5 p-4">
        <div className="mb-3 text-sm text-dark-5 dark:text-white/70">Daftar Akun</div>

        {loading ? (
          <div className="text-dark-5 dark:text-white/60">Loading...</div>
        ) : null}
